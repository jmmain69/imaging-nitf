...
public class FileComparer {
	...
	private void addCommonFileLevelMetadata(Map<String, String> metadata) throws IOException {
		...
		metadata.put("NITF_FSCLAS",
				nitfHeader.getFileSecurityMetadata().getSecurityClassification().getTextEquivalent());
	...
	private void addNITF21FileLevelMetadata(Map<String, String> metadata) throws IOException {
		NitfHeader nitfHeader = parseStrategy.getNitfHeader();
		metadata.put("NITF_FBKGC",
				(String.format("%3d,%3d,%3d", (int) (nitfHeader.getFileBackgroundColour().getRed() & 0xFF),
		...
		if (nitfHeader.getFileSecurityMetadata().getDeclassificationExemption().length() > 0) {
			metadata.put("NITF_FSDCXM",
					String.format("%4s", nitfHeader.getFileSecurityMetadata().getDeclassificationExemption()));
		} else {
			metadata.put("NITF_FSDCXM", "");
		}
	...
	private void addNITF20ImageSegmentMetadata(Map<String, String> metadata) throws IOException {
		...
		metadata.put("NITF_ICORDS",
				segment1.getImageCoordinatesRepresentation().getTextEquivalent(nitfHeader.getFileType()));
	...
	private void addNITF21ImageSegmentMetadata(Map<String, String> metadata) throws IOException {
		...
		if ((segment1.getSecurityMetadata().getDeclassificationExemption() != null)
			...
			metadata.put("NITF_ISDCXM",
					String.format("%4s", segment1.getSecurityMetadata().getDeclassificationExemption()));
		} else {
			metadata.put("NITF_ISDCXM", "");
		}
	...
	private void addCommonImageSegmentMetadata(Map<String, String> metadata) throws IOException, NitfFormatException {
		...
		if (segment1.getImageCoordinatesRepresentation() != ImageCoordinatesRepresentation.NONE) {
			metadata.put("NITF_IGEOLO",
					String.format("%s%s%s%s", segment1.getImageCoordinates().getCoordinate00().getSourceFormat(),
	...
	private void addRpfNamesMetadata(Map<String, String> metadata) throws IOException, NitfFormatException {
		if (filename.toLowerCase().endsWith(".ntf")) {
			// GDAL does this off the filename, not off the IID2, so it won't show these for
			// "plain" NITF files
			return;
		}
	...
	private boolean hasTREsOtherThanRPF(TreCollection treCollection) {
		...
		for (Tre tre : treCollection.getTREs()) {
			if ((tre.getName().equals("RPFHDR")) || (tre.getName().equals("RPFIMG"))
					|| (tre.getName().equals("RPFDES"))) {
				continue;
			}
			if (tre.getRawData() != null) {
				...
				if (tre.getName().equals("RPC00B")) {
					for (TreEntry entry : tre.getEntries()) {
						...
						if (entry.getFieldValue() != null) {
							if (entry.getName().equals("LONG_OFF") || entry.getName().equals("LONG_SCALE")
									|| entry.getName().equals("LAT_OFF") || entry.getName().equals("LAT_SCALE")) {
							...
							// These are too sensitive to number formatting issues, and we're already
							// checking
	...
	private void outputRPFDESmetadata(Map<String, String> metadata, Tre tre) {
		try {
			...
			if (attributes.getProductionDate() != null) {
				...
				if (!dateIndices.isEmpty()) {
					metadata.put("NITF_RPF_ProductionDate",
							attributes.getProductionDate(dateIndices.iterator().next()));
				}
			}
			if (attributes.getSignificantDate() != null) {
				...
				if (!dateIndices.isEmpty()) {
					metadata.put("NITF_RPF_SignificantDate",
							attributes.getSignificantDate(dateIndices.iterator().next()));
				}
			}
	...
	private static void outputThisEntry(BufferedWriter out, TreEntry entry, int indentLevel) throws IOException {
		if (entry.getFieldValue() != null) {
			...
			out.write(
					"<field name=\"" + entry.getName() + "\" value=\"" + rightTrim(entry.getFieldValue()) + "\" />\n");
		}
	...
	private static String makeGeoString(ImageCoordinatePair coords) {
		...
		return String.format("%02d%02d%02d%s%03d%02d%02d%s", latDegrees, latMinutes, latSeconds, northSouth, lonDegrees,
				lonMinutes, lonSeconds, eastWest);
	}
	...
	private void generateGdalMetadata() {
		try {
			...
			ProcessBuilder processBuilder = new ProcessBuilder("gdalinfo", "-nogcp", "-mdd", "xml:TRE",
			...
			BufferedReader infoOutputReader = new BufferedReader(
					new InputStreamReader(process.getInputStream(), "UTF-8"), 1000000);
			boolean done = false;
			try {
				do {
					do {
						...
						if (line.startsWith("  LINE_DEN_COEFF=") || line.startsWith("  LINE_NUM_COEFF=")
								|| line.startsWith("  SAMP_DEN_COEFF=") || line.startsWith("  SAMP_NUM_COEFF=")) {
							LOGGER.debug("Filtering out RPC coefficients");
							continue;
						}
						if (line.startsWith("  LAT_SCALE=") || line.startsWith("  LONG_SCALE=")
								|| line.startsWith("  LAT_OFF=") || line.startsWith("  LONG_OFF=")) {
							LOGGER.debug("Filtering out RPC coefficients");
							continue;
						}
